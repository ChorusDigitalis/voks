...
#ifndef  __effortFilter_H__
#define  __effortFilter_H__
...
#ifndef FAUSTCLASS 
#define FAUSTCLASS effortFilter
#endif
...
static double effortFilter_faustpower2_f(double value) {
	return value * value;
}

class effortFilter : public dsp {
 ...
 public:
	...
	void metadata(Meta* m) { 
		...
		m->declare("compile_options", "-a /usr/local/share/faust/max-msp/max-msp64.cpp -lang cpp -i -ct 1 -cn effortFilter -es 1 -mcd 16 -uim -double -ftz 0");
		m->declare("filename", "effortFilter.dsp");
	...
	virtual void instanceConstants(int sample_rate) {
		fSampleRate = sample_rate;
		fConst0 = std::cos(18849.55592153876 / std::min<double>(1.92e+05, std::max<double>(1.0, double(fSampleRate)))) + -1.0;
	}
	...
	virtual effortFilter* clone() {
		return new effortFilter();
	}
	...
	#define FAUST_FILE_NAME "effortFilter.dsp"
	#define FAUST_CLASS_NAME "effortFilter"
	#define FAUST_COMPILATION_OPIONS "-a /usr/local/share/faust/max-msp/max-msp64.cpp -lang cpp -i -ct 1 -cn effortFilter -es 1 -mcd 16 -uim -double -ftz 0"
 ...
 */
class effortFilter_poly : public dsp_voice_group, public dsp_poly {
    ...
    private:
        ...
        {
            if (val == FAUSTFLOAT(1)) {
                static_cast<effortFilter_poly*>(arg)->allNotesOff(true);
            }
        }
    ...
    public:
         ...
         * Constructor.
         *
         * @param dsp - the dsp to be used for one voice. Beware: effortFilter_poly will use and finally delete the pointer.
        ...
        virtual effortFilter_poly* clone()
        {
            return new effortFilter_poly(fDSP->clone(), int(fVoiceTable.size()), fVoiceControl, fGroupControl);
        }
...
struct dsp_poly_factory : public dsp_factory {
    ...
    {
        dsp_poly* dsp_poly = new effortFilter_poly(adaptDSP(fProcessFactory->createDSPInstance(), is_double), nvoices, control, group);
        if (fEffectFactory) {
...
void faust_allocate(t_faust* x, int nvoices)
{
    ...
    if (nvoices > 0) {
    ...
    #endif
        x->m_dsp = new effortFilter_poly(new effortFilter(), nvoices, true, true);
    #ifdef POLY2
        ...
        x->m_dsp = createSRAdapter<FAUSTFLOAT>(new effortFilter(), DOWN_SAMPLING, UP_SAMPLING, FILTER_TYPE);
    }
...
void* faust_new(t_symbol* s, short ac, t_atom* av)
{
    ...
    effortFilter* tmp_dsp = new effortFilter();
